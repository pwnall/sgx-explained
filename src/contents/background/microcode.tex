...
According to a 2014 patent \cite{intel2013scattergather}, the avenues
...
Microcode performs the processor's hardware initialization (\S~\ref{cpu_init}).
The SDM states that the BIST is performed by microcode. On some systems,
microcode also authenticates the boot firmware \cite{intel2012uefihypervisor}
before executing it. On systems that use LaGrande Server Extensions (LT-SX,
also known as Intel TXT for servers), the hardware initialization microcode
checks Intel's signature on the motherboard's firmware and assures the
motherboard that the CPU is compatible with LT-SX \cite{intel2012ltsx}. The
microcode in the SIPI (\S~\ref{sec:firmware_boot}) handler sets up a secure
environment matching the other CPUs.
