\subsection{Enclave Signature Verification}
...
Given that $s$ is the output of an RSA signature, its size must be at most the
size of the RSA key's modulus. It follows that

$q_1$ and $q_2$ are integer division quotients,
where the divi

Therefore, the size of $x$
