...
to access the PRM, so the page allocation instructions provide the only
...
enclave. It follows that enclave code can access the application's non-EPC
memory using the same virtual addresses as the rest of the application, which
facilitates moving application code that uses pointers into SGX enclaves.
...
\begin{figure}[hbt]
  \center{\includegraphics[width=85mm]{figures/sgx_mapping_attack.pdf}}
  \caption{
    Enclave data is stored into the EPC, which is a subset of the PRM. The
    PRM is a contiguous range of DRAM that cannot be accessed by system
    software or peripherals.
...
In a memory mapping attack, malicious system software would set up the page
tables or EPC in such a way that the virtual address intended to store the
...
SGX is able to prevent memory mapping attacks thanks to the \textit{Enclave
Page Cache Map} (EPCM), which contains security metadata for each EPC page,
summarized in Table~\ref{fig:epcm_entry}.
...
The metadata for a page includes the expected linear address used to access the
page (ENCLAVEADDRESS in Intel's documentation). This address must be specified
when a page is allocated, and cannot be changed until the page is freed.
...
The EPCM metadata also tracks the enclave that owns each page, so the processor
can prevent the code inside an enclave from accessing EPC pages belonging to
other enclaves.
