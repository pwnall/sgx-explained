...
lacks most of the sections that mention the PRM.
...
translation process and page tables (\S~\ref{sec:paging}) as the application
hosting the enclave.

% SGX Enclave Control Structure (SECS): SDM S 38.7

Each enclave designates an area in its virtual address space, called the
\textit{Enclave Linear Range} (ELRANGE), which is used to map the code and
the sensitive data stored in the enclave's EPC pages. The virtual address
...
Figure~\ref{fig:sgx_elrange}. This design facilitates the migration of
...
The system software managing a SGX computer is expected to set up each
enclave's page tables according to the enclave developer's design. However, the
system software is not trusted to do so. An SGX-enabled CPU prevents direct
active address translation attacks using the
\textit{Enclave Page Cache Map}~(EPCM), which has an entry containing the
security metadata shown in Table~\ref{fig:epcm_entry} for each EPC page.
