...
SGX prevent memory mapping attacks using the \textit{Enclave Page Cache Map}
(EPCM), which contains security metadata for each EPC page.
Table~\ref{fig:epcm_entry} lists the metadata stored in the EPCM.
...
The metadata for a page includes the expected virtual address
(\S~\ref{sec:segments}) used to access the page (ENCLAVEADDRESS in Intel's
documentation). This address must be specified when a page is allocated, and
cannot be changed until the page is freed.
...
that the virtual address used in the address translation matches the expected
virtual address recorded in the page's EPCM entry. This prevents the system
...
specify the enclave's virtual address space (ELRANGE, meaning enclave linear
address range, in the Intel
documentation). The enclave's starting address has to be aligned by the enclave
size, which simplifies checking if a virtual address belongs to an enclave or
